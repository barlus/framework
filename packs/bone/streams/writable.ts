import {Strategy,Sink} from './interfaces';
...
export class WritableStream<T> {
    ...
    /**
     * @returns The stream's current @WritableState
     */
    get state(): WritableState {
        return this._state;
    }
    ...
    protected _state: WritableState;
    ...
    constructor(underlyingSink: Sink<T> = {}, strategy: Strategy<T> = {}) {
        ...
        this._state = WritableState.Writable;
        this._started = false;
        this._writing = false;
    ...
    protected _advanceQueue() {
        ...
        if (this._queue.length === 0) {
            ...
            if (writeRecord && writeRecord.close) {
                ...
                if (this.state !== WritableState.Closing) {
            ...
            util.promiseInvokeOrNoop(this._underlyingSink, 'write', [chunk]).then(() => {
                if (this.state !== WritableState.Errored) {
                    this._writing = false;
    ...
    protected _close(): void {
        ...
        util.promiseInvokeOrNoop(this._underlyingSink, 'close').then(() => {
            if (this.state !== WritableState.Errored) {
                ...
                this._state = WritableState.Closed;
    ...
    protected _error(error: Error) {
        if (this.state === WritableState.Closed || this.state === WritableState.Errored) {
            return;
        }
        ...
        if (this.state === WritableState.Waiting) {
            this._resolveReadyPromise();
        }

        this._rejectClosedPromise(error);
        this._state = WritableState.Errored;
    }
    ...
    protected _syncStateWithQueue(): void {
        ...
        if (shouldApplyBackPressure && this.state === WritableState.Writable) {
            this._state = WritableState.Waiting;
        ...
        if (shouldApplyBackPressure === false && this.state === WritableState.Waiting) {
            this._state = WritableState.Writable;
            this._resolveReadyPromise();
        }
    }
    ...
    abort(reason: any): Promise<void> {
        ...
        if (this.state === WritableState.Closed) {
            // 4.2.4.4-2
    ...
    close(): Promise<void> {
        ...
        this._state = WritableState.Closing;
