...
export class WebSocket extends EventEmitter {
  ...
  protocol: string;
  private _binaryType;
  private _closeFrameReceived;
  private _closeFrameSent;
  ...
  addEventListener(...args) {
    return EventTarget.addEventListener.call(this, ...args)
  }
  removeEventListener(...args) {
    return EventTarget.removeEventListener.call(this, ...args)
  }
}
...
function initAsClient(address, protocols, options) {
  ...
  req.on('upgrade', (res, socket, head) => {
    ...
    const digest = Crypto.createHash('sha1')
      .update(Buffer.from(key + constants.GUID, 'binary'))
      .digest('base64');
