...
function isInitialState<E = any>(state: AbstractState<E>): state is Initial<E> {
  return state instanceof Initial
}
function isHistoryState<E = any>(state: AbstractState<E>): state is History<E> {
  return state instanceof History
}
function isFinalState<E = any>(state: AbstractState<E>): state is Final<E> {
  return state instanceof Final
}
function isExclusiveState<E = any>(state: AbstractState<E>): state is State<E> {
  return state instanceof State
}
function isParallelState<E = any>(state: AbstractState<E>): state is Parallel<E> {
  return state instanceof Parallel
}

function isRealState<E = any>(state: AbstractState<E>): state is RealState<E> {
  return isFinalState(state) || isCompositeState(state)
}
function isTargetState<E = any>(state: AbstractState<E>): state is TargetState<E> {
  return isRealState(state) || isHistoryState(state)
}
function isCompositeState<E = any>(state: AbstractState<E>): state is CompositeState<E> {
  return isParallelState(state) || isExclusiveState(state)
}
...
class Interpreter<EventType> {
  ...
  private addAncestorStatesToEnter(state: RealState<EventType>, ancestor: CompositeState<EventType>,
                                   ...
                                   statesForDefaultEntry: Set<State<EventType>>
  ): void {
  ...
  private computeEntrySet(
    enabledTransitions: Transition<EventType>[],
    statesToEnter: Set<RealState<EventType>>,
    statesForDefaultEntry: Set<State<EventType>>
  ): void {
  ...
  private isInFinalState(state: CompositeState<EventType>): boolean {
    if (this.isCompound(state)) {
      return state.childStates.some(s => {
        return isFinalState(s) && this.configuration.has(s)
      })
