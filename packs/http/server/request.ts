...
import {only} from '../utils/objects';
import {typeis} from '../content/content-type-is';
import {URL, format } from '@barlus/node/url';
import {isIP} from '@barlus/node/net';
import {parse,stringify} from '@barlus/node/querystring';
import {fresh} from '../utils/fresh';
...
export class Request {
  ...
  get charset() {
    // todo fix typeing
    let type = this.get('Content-Type') as any;
    if (!type) {
      return '';
    }
