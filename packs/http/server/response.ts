...
import {statuses, empty, redirect} from '../statuses';
import {Stream} from '@barlus/node/stream';
import {Buffer} from '@barlus/node/buffer';
import {extname} from '@barlus/node/path';
import {contentDisposition} from '../content/disposition'
import {typeis} from '../content/content-type-is'
import {contentType} from '../content/mime-types'
import {escapeHtml} from '../utils/html-escape'
import {onFinished} from '../utils/on-finished'
import {destroy} from '../utils/destroy'
import {vary} from '../content/vary'
...
export class Response {
    ...
    set status(code) {
        ArgumentError.assert('number' == typeof code, 'status code must be a number');
        ArgumentError.assert(statuses[code], `invalid status code: ${code}`);
        ArgumentError.assert(!this.res.headersSent, 'headers have already been sent');
        this._explicitStatus = true;
        this.res.statusCode = code;
        if (this.req.httpVersionMajor < 2) {
            this.res.statusMessage = statuses[code];
        }
        if (this.body && empty[code]) {
            this.body = null;
        }
    }
    /**
