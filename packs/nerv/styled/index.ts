...
const dimensionptn = /stretch|:\s*\w+\-(?:conte|avail)/;
...
function select(parent, current, invert) {
    const selectors = current.trim().split(selectorptn);
    let i, j, selector, out = selectors;
    ...
    switch (l) {
        ...
        case 0:
        case 1: {
            for (i = 0, selector = l === 0 ? '' : parent[ 0 ] + ' '; i < length; ++i) {
...
function vendor(content, context) {
    const index = content.indexOf(context === 1 ? ':' : '{');
    const key = content.substring(0, context !== 3 ? index : 10);
    const value = content.substring(index + 1, content.length - 1);
...
function supports(match, group) {
    const out = property(group, group.charCodeAt(0), group.charCodeAt(1), group.charCodeAt(2));
...
function animation(input) {
    ...
    const declare = input.substring(0, index).trim();
    ...
    switch (input.charCodeAt(9) * keyed) {
        ...
        default: {
            ...
            const list = out.split((out = '', animationptn));

            for (i = 0, index = 0, length = list.length; i < length; index = 0, ++i) {
                let value = list[ i ];
                const items = value.split(propertiesptn);

                while (value = items[ index ]) {
                    const peak = value.charCodeAt(0);
                    ...
                    if (keyed === 1 && (
                        ...
                        switch ((isNaN(parseFloat(value)) ? 1 : 0) + (value.indexOf('(') !== -1 ? 1 : 0)) {
                            case 1: {
                                switch (value) {
...
function isolate(current) {
    for (var i = 0, length = current.length, selector = Array(length), padding, element; i < length; ++i) {
        ...
        const elements = current[ i ].split(elementptn);
        let out = '';

        let j = 0, size = 0, tail = 0, code = 0;
...
function stylis(selector, input) {
    ...
    // build
    let output = compile(array, selectors, input, 0, 0);
...
function styled(str) {
    return (target)=>{
        console.info(stylis('.'+target.name,str[0]));
    }
}
