    ...
    strings.forEach((str, index) => {
        if (keys.length >= index) {
            ...
            if (typeof keyValue === 'function') {
                ...
                if (keyValue instanceof HTMLElement) {
                    str += '.'+keyValue.className;
                    keyValue='';
                }else{
...
export function buildName(hash, isKeyframes = false) {
...
function buildAndRenderCSS(strings, keys, state, isKeyframes = false) {

    const rawCSS = joinTemplate(strings, keys, state);
    const hash = hashed(rawCSS).toString(36);
...
namespace styled {
    export declare const div: (...args) => (props, ...children: (string | Element)[]) => Element;
    export declare const h2: (...args) => (props, ...children: (string | Element)[]) => Element;
    export declare const button: (...args) => (props, ...children: (string | Element)[]) => Element;

    export const setTheme = selectedTheme => (theme = Object.assign({}, selectedTheme));
    export const css = (strings, ...keys) => buildAndRenderCSS(strings, keys, { theme });
    export const injectGlobal = (strings, ...keys) => {
        globalCSS += joinTemplate(strings, keys, { theme });
    };
    export const keyframes = makeKeyframes;
    export const tags = [
