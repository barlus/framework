 ...
 */
let registrationNameDependencies: any = {};
 ...
 * @type {Object}
 */
let possibleRegistrationNames: any = {};
 ...
 */
let emptyFunction: any = function emptyFunction() {
};
...
let shouldBeReleasedProperties:any[] = [ 'dispatchConfig', '_targetInst', 'nativeEvent', 'isDefaultPrevented', 'isPropagationStopped', '_dispatchListeners', '_dispatchInstances' ];
 ...
 */
var SyntheticEvent = function SyntheticEvent(dispatchConfig, targetInst, nativeEvent, nativeEventTarget) {
    {
...
_assign(SyntheticEvent.prototype, {
    ...
    stopPropagation: function () {
        let event = this.nativeEvent as any;
        if (!event) {
            return;
        }
...
_assign(SyntheticEvent, {
    ...
    extend: function (Interface) {
        ...
        (Class as any).Interface = _assign({}, Super.Interface, Interface);
        (Class as any).extend = Super.extend;
...
let SyntheticEvent$1: any = SyntheticEvent;
...
let ReactInternals$1:any = React.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
...
if (ExecutionEnvironment_1.canUseDOM) {
    ...
    isInputEventSupported = isEventSupported('input') && (!(document as any).documentMode || (document as any).documentMode > 9);
}
        ...
        if (!(isListening.hasOwnProperty(dependency) && isListening[ dependency ])) {
            switch (dependency) {
                ...
                default:
                    ...
                    let isMediaEvent = mediaEventTypes.indexOf(dependency) !== -1;
                    if (!isMediaEvent) {
...
let skipSelectionChangeEvent = ExecutionEnvironment_1.canUseDOM && 'documentMode' in document && (document as any).documentMode <= 11;
let eventTypes$3 = {
    select: {
        phasedRegistrationNames: {
            bubbled: 'onSelect',
...
if (!ExecutionEnvironment_1.canUseDOM) {
    ...
    scheduleWork = function (callback, options) {
        let callbackId = callbackIdCounter++;
    ...
    cancelScheduledWork = function (callbackId) {
        let timeoutId = timeoutIds[ callbackId ];
    ...
    let safelyCallScheduledCallback = function (callback, callbackId) {
    ...
    let callTimedOutCallbacks = function () {
        ...
        let i = 0;
        let len = pendingCallbacks.length;
        for (; i < len; i++) {
            let currentCallbackConfig = pendingCallbacks[ i ];
            let _timeoutTime = currentCallbackConfig.timeoutTime;
            if (_timeoutTime !== -1 && _timeoutTime <= currentTime) {
                // it has timed out!
                // call it
                let _callback = currentCallbackConfig.scheduledCallback;
    ...
    let messageKey = '__reactIdleCallback$' + Math.random().toString(36).slice(2);
    let idleTick = function (event) {
        ...
        while (frameDeadline - currentTime > 0 && pendingCallbacks.length > 0) {
            let latestCallbackConfig = pendingCallbacks.shift();
            frameDeadlineObject.didTimeout = false;
            let latestCallback = latestCallbackConfig.scheduledCallback;
            let newCallbackId = latestCallbackConfig.callbackId;
    ...
    scheduleWork = function (callback, options) {
        ...
        let newCallbackId = getCallbackId();
...
let validateProperty$1:any = function () {
};
...
let cloneInstance: any = shim;
let createContainerChildSet: any = shim;
let appendChildToContainerChildSet: any = shim;
let finalizeContainerChildren: any = shim;
let replaceContainerChildren: any = shim;
...
let fireGetDerivedStateFromPropsOnStateUpdates = true;
...
let resumeTimersRecursively = function (fiber) {
 ...
 * same logic and follow the same code paths.
 */
let lowPriorityWarning:any = function () {
};
...
let lowPriorityWarning$1:any = lowPriorityWarning;
    ...
    fiber.effectTag |= Placement;
    {
        switch (returnFiber.tag) {
            ...
            case HostComponent: {
                ...
                switch (fiber.tag) {
                    case HostComponent:
                        let _type = fiber.type;
                        let _props = fiber.pendingProps;
                        didNotFindHydratableInstance(parentType, parentProps, parentInstance, _type, _props);
                        break;
                    case HostText:
                        let _text = fiber.pendingProps;
    ...
    while (fiber !== null) {
        ...
        switch (fiber.tag) {
            case ContextConsumer:
                ...
                let observedBits = fiber.stateNode | 0;
...
let invokeGuardedCallback$3:any = ReactErrorUtils.invokeGuardedCallback;
let hasCaughtError$1:any = ReactErrorUtils.hasCaughtError;
let clearCaughtError$1:any = ReactErrorUtils.clearCaughtError;
...
let invokeGuardedCallback$2: any = ReactErrorUtils.invokeGuardedCallback;
