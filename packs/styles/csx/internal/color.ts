...
import {roundFloat}       from "../utils/index";

import {StringType, NamedColor} from '../types';

export type ColorLike = ColorHelper | string;
...
export function isColor(stringValue: ColorLike): stringValue is ColorLike {
  if (stringValue instanceof ColorHelper) {
    return true;
  }
  return !!namedColors[stringValue] || !!stringValue.match(/#{6}|#{8}/)
}
 ...
 */
export function color(value: NamedColor | string): ColorHelper {
...
export class ColorHelper implements StringType<ColorProperty> {
  ...
  public mix(mixin: ColorProperty | ColorHelper, weight?: number): ColorHelper {
    const color1 = this;
...
function ensureColor(c: ColorProperty | ColorHelper): ColorHelper {
