...
import {FunctionSpyCallCountMatcher} from "./FunctionSpyCallCountMatcher";
import {SpyCallCountType}            from "./SpyCallCountType";


export class FunctionSpyMatcher {

  private _spy: FunctionSpy;
  private _expectedArguments: any[] | null = null;

  public constructor(spy: FunctionSpy, expectedArguments?: any[]) {
  ...
  public lessThan(maximumCallCount: number): FunctionSpyCallCountMatcher {
    ...
    if (this._expectedArguments && this._getArgumentsMatching(this._expectedArguments).length >= maximumCallCount) {
      throw new FunctionCallCountMatchError(
        this._spy,
        true,
        maximumCallCount,
  ...
  private _getArgumentsMatching(expectedArguments: any[]) {
