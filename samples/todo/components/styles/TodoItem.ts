import { stylesheet,css,media,MediaQuery } from '@barlus/styles';
...
stylesheet(`styles/${Theme.TodoItem}.tcss`)(`.${Theme.TodoItem}`,{
    ...
    ...css.nest(`&.${Theme.TodoItemEditing}`,{
        borderBottom:'none',
        padding:0,
        ...css.nest(`& .${Theme.TodoEntry}`,{
            display:'block',
    ...
    ...css.nest(`& .${Theme.TodoItemCompleted} label`,{
        color:'#d9d9d9',
        textDecoration: 'line-through'
    }),
    ...css.nest(`& .${Theme.TodoItemDestroy}`,{
        display: 'none',
        position: 'absolute',
        top: 0,
        right: 10,
...
media({type:'screen'/*,'-webkit-min-device-pixel-ratio':0*/},{
    ...
    ...css.nest(`.${Theme.TodoItem} ${Theme.TodoItemToggle}`,{
        background:'none',
        height:40
    }),
});